...
\subsection{Introduction to OpenStack}
...
downloading the environment file called openrc.sh file. This file provides
project-specific contains the credentials that the OpenStack services can use.
It can be downloaded from the web interface via the \textit{Access \& Security}
link, then clicking on the \textit{API Access} tab and clicking on
\textit{Download OpenStack RC File} on the top of the
page~\cite{hid-sp18-516-www-openrc}. 
...
\begin{description}
   ...
   uncompressed and treated like if were not compressed. Compression of data 
...
creation and saved to an output file called \textit{output.txt} in the user's
home directory. This user data is saved in the util.py file.
...
The following table lists the libcloud functions that were leveraged during the
development of the REST APIs~\cite{hid-sp18-516-www-libcloud-functions}. 
...
downloading swagger-codegen jar file, setting p the work directory, copying all
the files to the desired work directory and starting the swagger service. 
...
username, password, keypairname is sensitive information, it needs to be
configured in this file before execution. Default parameters for image, flavor,
security group, can also be specified in this file.
...
can be deployed using Apache libcloud. We see how Apache libcloud can be
